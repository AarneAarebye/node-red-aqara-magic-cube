...
| [`flows/magic-cube-homekit-parser.js`](flows/magic-cube-homekit-parser.js) / [`.flow.json`](flows/magic-cube-homekit-parser.flow.json) | 12 (one per gesture, plus side and battery) | You want to wire straight into NRCHKB yourself, with your own accessory layout, and only have one cube. |
| [`flows/magic-cube-homekit-subflow.flow.json`](flows/magic-cube-homekit-subflow.flow.json) | 12 (same as above) | Same as above, but you have **more than one cube**: this packages the same logic as a reusable Node-RED subflow, with every tunable (stale-action suppression, refire window, low-battery threshold, PSE cycling) exposed as a per-instance property — no code editing, no forking the file per cube. |
